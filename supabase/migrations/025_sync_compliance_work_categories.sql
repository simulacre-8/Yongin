-- Keep legacy/runtime classification consistent with the UI default recommendation.
-- Priority is deliberate: a planning duty remains PLAN even if its title also contains inspection;
-- a contractor inspection remains CONTRACT; users can still choose another category when recording.
...
with classified as (
  ...
  where dae.actor_employee_no in ('DEMO-UNASSIGNED', 'DEMO-SEED')
), updated as (
