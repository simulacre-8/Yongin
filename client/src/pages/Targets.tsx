...
export default function Targets() {
  ...
  const filtered = useMemo(
    () =>
      targets.filter(item =>
        `${item.name} ${item.department} ${item.type}`.includes(query)
      ),
    [query]
  );
  const selected =
  ...
  return (
    <div className="page">
      <div className="page-heading">
        <div>
          <span className="eyebrow">MANAGED TARGETS</span>
          <h1>관리대상 현황</h1>
          <p>담당 시설의 기본정보와 법 의무사항 등록 현황을 조회합니다.</p>
        </div>
      </div>
      <div className="search-panel">
        ...
        <label className="search-input">
          ...
          <input
            ...
            placeholder="시설물명 또는 부서명을 입력하세요"
          />
          <Search size={17} />
        </label>
      ...
      <div className="split-layout">
        <section className="data-panel grow">
          <div className="panel-bar">
            <strong>
              총 <b>{filtered.length}</b>개소
            </strong>
            <span>입력완료 {filtered.length}개소</span>
          </div>
        ...
        <aside className="detail-panel">
          <span className="detail-kicker">선택 대상</span>
          <h2>{selected.name}</h2>
          ...
          <dl>
            ...
            <div>
              ...
              <dd>
                {selected.department} / {selected.manager}
              </dd>
            </div>
          </dl>
          ...
          <div className="detail-actions">
            <button className="secondary-btn" onClick={() => navigate("/laws")}>
              관계 법령 조회
            </button>
            <button
