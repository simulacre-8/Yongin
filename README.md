...
현재 검수용 GNB는 `홈`, `이행현황`, `관리대상 현황`, `의무사항`, `의무이행`, `이행점검`, `설정` 순서입니다. 메뉴명과 중처법 의무이행 프로시저는 다음 검수 단계에서 다시 확정합니다. 홈은 본문 안의 **중처법 카테고리별**과 **중대재해 유형별** 두 구역만 로컬 내비게이션으로 유지합니다. 중대산업재해는 14개 하위 메뉴를 다시 펼치지 않고, 오른쪽 의무 행 아래에 법령 원문의 세부 점검사항을 번호와 들여쓰기로 표시합니다. 의무 행을 선택하면 실제 workflow 상태의 이행률·O/△/X/- 집계와 조직 배정 현황을 확인할 수 있습니다.
