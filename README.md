...
시각 기준은 **검정 GNB 유지, 내부 회색 베이스, 용인특례시 핑크 강조**입니다. 본문·표·입력·버튼은 전역 확대를 적용하지 않고 각 원본형 화면의 폰트 크기를 사용합니다. 현재 크기와 사용 예시는 `docs/FONT_SIZE_SPEC.md`에 정리했습니다. 기존 접이식 LNB, 용인시청 단일 대상, Supabase ADOMS 조회·판정 저장, 시연 초기화 기능은 유지합니다.
