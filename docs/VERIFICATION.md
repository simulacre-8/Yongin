...
| 셸·증빙 UI                | 전역 LNB 제거·블루톤·`+ 첨부`·이미지 뷰어 제거 확인       |
...
현재 검수용 GNB는 `홈 → 관리대상 → 의무 체크리스트 → 의무이행 → 설정` 순서다. `이행현황`과 `이행점검`은 GNB에서 숨기고 기존 라우트는 후속 업무 흐름 검수용으로 유지한다. DB 연결 상태, 역할 전환, 초기화는 설정 오른쪽의 한 행에 배치해 헤더 높이를 줄였다. 메뉴명과 중처법 프로시저는 다음 검수 단계에서 다시 확정한다.

설정 화면으로 대상 프로필 사실값을 이동했다. 브라우저에서 120명·39,872㎡·기준일 2026-09-05를 저장한 뒤, 원격 `target_applicability`의 `RUL-000840`, `RUL-000841`, `RUL-000900`, `RUL-000901` 네 행에서 같은 입력 스냅숏과 기록시각을 재조회했다. 적용범위 화면에서는 프리셋·사실값 입력·용인시청 기본값 영역을 제거하고 승인규칙 31개 중 현재 실행규칙 4개의 L1/L2/L3 결과와 근거만 유지했다.
...
현재 원격 DB에는 기획조정실·행정국·중대재해예방팀 조직 및 대상·의무 배정 데이터가 없다. 상세 화면은 숫자를 임의 생성하지 않고 조직 셀을 `미배정`으로 표시한다. 대표 브라우저 검증 `OBL-0000003`은 연결 관리대상 2개, 상태 `NA` 2건, 이행률 0.0%로 조회됐으며 콘솔 오류는 없었다.
