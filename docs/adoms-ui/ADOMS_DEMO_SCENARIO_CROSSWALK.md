...
**갱신된 제약.** 적용범위 첫 화면은 용인시청 기본값을 사용한다. 관리대상 조회 범위는 클라이언트가 제공한 FMS 시설 150건과 별도 분류한 공중교통수단 1건·도급 2건으로 확장됐다. 법령/의무 기준정보는 읽기 전용으로 통제하고, 적용판정 근거 snapshot과 이행/증빙/점검/조치 상태는 영속 저장한다.
