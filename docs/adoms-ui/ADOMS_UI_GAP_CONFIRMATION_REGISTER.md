...
**범위.** 이 등록부는 일반 개선 제안이 아니라 **화면 부재, 라벨 충돌, 대상 충돌, 권한 불확실, 신규 제작 필요**만 기록한다. 우선순위는 용인시청 **단일 대상**, Supabase ADOMS 투영, 적용판정 영속 저장 시연의 완주 가능성으로 정했다.
...
|---|---|---|---|---|
| GAP-P0-01 | 대상 충돌 / 신규 제작 | SCR-034~035, 057~074; ACT1-03~04 | “용인경전철을 독립 `공중교통수단` target type으로 신설합니까, 또는 원본의 `공중이용시설·교통수단` 하위 subtype으로 저장합니까? 도시철도차량·노선연장·차량편성·운영주체·관리부서의 필수값과 유효성 기준을 확정해 주십시오.” | 등록 탭/CTA를 만들지 않고 검수 시드만 읽기전용으로 둔다. |
