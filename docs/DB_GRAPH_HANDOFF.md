...
현재 제공된 publishable/anon 키는 브라우저에서 RLS가 허용한 데이터를 읽고 쓰기 위한 공개키다. 스키마 생성 권한은 없으므로 담당자는 Supabase Dashboard의 SQL Editor에서 **README `Supabase 적용`에 명시된 전체 마이그레이션 001~021과 후행 시드를 순서대로 실행**한다. 기존 이행·증빙의 시정조치 백필을 위해 `supabase/seed_compliance_action_runtime.sql`은 모든 업무 시드 뒤에 실행한다.
...
| ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- | ---------------------------- | ------------------------------------------------------------------ |
| 호스팅형 Supabase PostgreSQL 단독        | 화요일까지 가장 단순하며 CRUD·Storage·RLS·집계에 적합하다. 다단계 경로 탐색은 재귀 SQL이나 관계 테이블로 처리한다.         | 기존 Supabase 프로젝트 범위  | 낮음: 버전 관리된 마이그레이션·시드 순차 적용과 공개 환경변수 등록 |
