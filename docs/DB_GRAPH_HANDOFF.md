...
화요일 시연 UI는 그래프 탐색보다 CRUD, 파일, 점검 상태 변경과 집계가 중심이므로 **호스팅형 Supabase PostgreSQL만으로 충분**하다. Docker는 필요 없다. 다만 원천의 `law_id`, `unit_id`, `rul_id`, `obl_id`와 관계 테이블을 보존하면 이후 그래프 DB로 옮겨도 데이터 재가공을 최소화할 수 있다.
